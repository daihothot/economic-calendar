...
首批数据包含 2026 年全球核心日程，以及已由美联储、欧洲央行和日本银行公布的 2027 央行日程。2027 文件暂标为 `partial`；距离新年 120 天以内时，覆盖检查会发出维护警告但不会把它误判为完整，BLS、BEA、Eurostat、国家统计局等机构发布年度表后再补齐，不提前猜日期。年度数据保存在 `data/events-YYYY.json`，新增年份后主订阅地址保持不变。
...
仓库每周检查下一年度覆盖范围；下一年度文件缺失会阻断检查，文件已存在但仍为 `partial` 时只发出维护警告，等待官方年度表发布后再补齐。发布日程可能被机构临时调整，因此事件说明始终保留核对日期和“以官方最新公告为准”。
